function other_cat() {
    ...
    var id = category_id;
    type = $("#first_category").find("option:selected").attr("data-type");
    select();
    mainFile();
    console.log(type);
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.other_categories.forEach((el) => {
                    response.other_categories_project.forEach((el2) => {
                        var output = `<option value="${el.id}" ${
                            el2.category_id == el.id ? "selected" : ""
                        }>${el.name}</option>`;
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.success == true) {
                $("#preview_image").html("");
                $("#preview_image").append(`<option value="0">Select</option>`);
                if (project_id > 0) {
                    response.files.forEach((el) => {
                        $("#preview_image").append(
                            `<option value="${el.id}" ${
                                el.id == cover_id ? "selected" : ""
                            } >${el.original_name} | ${el.created_at}</option>`
                        );
                    });
                } else {
                    response.files.forEach((el) => {
...
function previewFile() {
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.success == true) {
                $("#preview_file").html("");
                $("#preview_file").append(`<option value="0">Select</option>`);
                if (project_id > 0) {
                    response.files.forEach((el) => {
                        $("#preview_file").append(
                            `<option value="${el.id}" ${
...
$(".btn_send").click(function (e) {
    ...
    if (tags.val() == "") {
        $(".bootstrap-tagsinput").attr("style", "border-color: red !important");
    }
